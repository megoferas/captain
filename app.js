...
/* ---------- monthly table: one employee, every day, exemption + same-day events ---------- */
function AdmMonthList() {
  const [staff, setStaff] = useState(null);
  useEffect(() => {
    sb.from('staff').select('id,full_name,code').eq('role', 'employee').eq('active', true).order('full_name')
      .then(({ data }) => setStaff(data || []));
  }, []);
  return html`<div class="page">
    <${Hero} title="الجدول الشهري" back="dash" slim />
    <div class="body flat">
      <div class="note info">جدول كل موظف على مدار الشهر: معاد الحضور كل يوم، والسلف والمكافآت والخصومات في يومها، وتقدر تعفي أي يوم متأخر أو غايب من هنا.</div>
      <div class="card tight">
        ${staff === null ? html`<div class="empty">لحظة...</div>` : staff.length === 0 ? html`<div class="empty">مفيش موظفين.</div>` : staff.map((s) => html`
          <a class="list-row" href=${'#month/' + s.id}>
...
function DaySheet({ row, name, canEdit, onClose, onExcuse }) {
  const st = STATUS[row.status] || STATUS.absent;
  return html`<${Sheet} title=${name} onClose=${onClose}>
    <div class="soft">${fmtDay(keyToDate(row.work_date))}</div>
    <div class="spread">
      <${Pill} tone=${st.tone}>${st.label}<//>
      ${row.check_in && html`<div class="soft num">${fmtTime(row.check_in)}${row.check_out ? ' — ' + fmtTime(row.check_out) : ' · لسه شغال'}</div>`}
    </div>
    ${row.status === 'late' && !row.late_excused && html`<div class="note warn">اتأخر ${row.late_minutes} دقيقة عن معاد الشفت.</div>`}
    ${row.late_excused && html`<div class="note ok">التأخير اتلغى.</div>`}
    ${row.absence_excused && html`<div class="note ok">الغياب اتلغى.</div>`}
    ${Array.isArray(row.events) && row.events.length > 0 && html`<div class="card tight" style="margin:0;padding:4px 0">
      <div class="h2" style="padding:8px 0 4px">أحداث اليوم ده</div>
      ${row.events.map((e) => html`<div class="list-row">
        <${Tile} icon=${e.type === 'advance' ? 'down' : e.type === 'bonus' ? 'gift' : 'minus'} tone=${e.type === 'deduction' ? 'red' : 'green'} sm />
        <div class="grow"><div style="font-weight:500">${e.type === 'advance' ? 'طلب سلفة' : e.type === 'bonus' ? 'مكافأة' : 'خصم'}${e.reason ? ' · ' + e.reason : ''}</div>
          ${e.status && html`<div class="soft">${e.status === 'pending' ? 'قيد المراجعة' : e.status === 'approved' ? 'مقبولة' : 'مرفوضة'}</div>`}</div>
        <div class=${'amt ' + (e.type === 'deduction' ? 'neg' : 'pos')}>${(e.type === 'deduction' ? '−' : '+') + fmtMoney(e.amount)}</div></div>`)}
    </div>`}
    ${canEdit && row.status === 'late' && !row.late_excused && html`<button class="btn ghost" onClick=${() => onExcuse('late')}><${Icon} name="check" size=${20} /> إلغاء التأخير</button>`}
    ${canEdit && row.status === 'absent' && !row.absence_excused && html`<button class="btn ghost" onClick=${() => onExcuse('absence')}><${Icon} name="check" size=${20} /> إلغاء الغياب</button>`}
  <//>`;
}

function AdmMonthCal({ id, month }) {
  const m = month ? monthStart(month) : monthStart(todayKey());
  const [s, setS] = useState(null);
  const [rows, setRows] = useState(null);
  ...
  async function load() {
    const [a, b] = await Promise.all([
      sb.from('staff').select('*').eq('id', id).maybeSingle(),
      sb.rpc('admin_month_grid', { p_month: m }),
    ]);
    setS(a.data);
    if (b.error) { toast(errText(b.error.message), 'bad'); setRows([]); return; }
    setRows((b.data || []).filter((r) => r.staff_id === id));
  }
  useEffect(() => { setRows(null); load(); }, [id, m]);
  const canEdit = can('attendance.edit');
  const atCurrent = m >= monthStart(todayKey());
  const cnt = { present: 0, late: 0, absent: 0, leave: 0, excused: 0 };
  (rows || []).forEach((r) => { if (cnt[r.status] != null) cnt[r.status] += 1; });
  return html`<div class="page nonav">
    <${Hero} title=${s ? s.full_name : 'الجدول الشهري'} sub=${monthName(m)} back="month" slim />
    <div class="body flat">
      <div class="card"><div class="spread">
        <button class="iconbtn light" aria-label="الشهر اللي بعده" onClick=${() => { location.hash = '#month/' + id + '/' + addMonths(m, 1); }} disabled=${atCurrent}><${Icon} name="chev" size=${22} /></button>
        <div class="h2">${monthName(m)}</div>
        <button class="iconbtn light" aria-label="الشهر اللي قبله" onClick=${() => { location.hash = '#month/' + id + '/' + addMonths(m, -1); }}><${Icon} name="back" size=${22} /></button>
      </div></div>
      ${rows && rows.length > 0 && html`<div class="chips" style="gap:10px">
        <span class="pill tone-green">${cnt.present} حضور</span>
        <span class="pill tone-amber">${cnt.late} تأخير</span>
        <span class="pill tone-red">${cnt.absent} غياب</span>
        <span class="pill tone-slate">${cnt.leave} إجازة</span>
      </div>`}
      <div class="card tight">
        ${rows === null ? html`<div class="empty">لحظة...</div>` : rows.length === 0 ? html`<div class="empty">مفيش بيانات للشهر ده لحد دلوقتي.</div>` : rows.map((r) => {
          const st = STATUS[r.status] || STATUS.absent;
          const hasEvents = Array.isArray(r.events) && r.events.length > 0;
          return html`<button type="button" class="list-row" onClick=${() => setDetail(r)}>
            <div class=${'avatar tone-' + st.tone}>${keyToDate(r.work_date).getUTCDate()}</div>
            <div class="grow"><div style="font-weight:600">${fmtShort(keyToDate(r.work_date))}</div>
              <div class="soft num">${r.check_in ? fmtTime(r.check_in) + (r.check_out ? ' — ' + fmtTime(r.check_out) : ' · لسه شغال') : st.label}</div></div>
            <div class="stack" style="align-items:flex-end;gap:4px">
              ${r.status === 'late' && !r.late_excused && html`<${Pill} tone="amber">${r.late_minutes} د<//>`}
              ${(r.late_excused || r.absence_excused) && html`<${Pill} tone="green">اتلغى<//>`}
              ${hasEvents && html`<${Pill} tone="sand">${r.events.length} حدث<//>`}
            </div></button>`;
        })}
      </div>
    </div>
    ${detail && html`<${DaySheet} row=${detail} name=${s ? s.full_name : ''} canEdit=${canEdit}
      onClose=${() => setDetail(null)}
      onExcuse=${(kind) => { setEx({ staffId: id, name: s ? s.full_name : '', date: detail.work_date, kind }); setDetail(null); }} />`}
    ${ex && html`<${ExcuseSheet} ...${ex} onClose=${() => setEx(null)} onDone=${() => { setEx(null); load(); window.dispatchEvent(new Event('adm-refresh')); }} />`}
  </div>`;
}
...
function AdminShell({ route }) {
  ...
  else if (head === 'month' && parts[1]) { view = gate(can('attendance.view'), html`<${AdmMonthCal} id=${parts[1]} month=${parts[2]} key=${parts[1] + (parts[2] || '')} />`); tab = 'dash'; hideNav = true; }
  else if (head === 'month') { view = gate(can('attendance.view'), html`<${AdmMonthList} />`); tab = 'dash'; }
  ...
  return html`<div style="height:100%"><${Boundary} key=${route}>${view}<//>${!hideNav && html`<${Nav} items=${navItems} tab=${tab} />`}</div>`;
}
